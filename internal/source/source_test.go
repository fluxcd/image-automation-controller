...
func TestSourceManager_CheckoutSource(t *testing.T) {
	// TODO: Test for both ssh and http.
	tests := []struct {
		name        string
		...
		// TODO: Add more tests for different arguments like auth secret, proxy
		// secret, cross namespace reference, different kind, missing gitspec,
		// etc. Or have another separate test for them.
		// - shallow clone
		// - with last observed commit
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			g := NewWithT(t)
			...
			_ = testutil.InitGitRepo(g, gitServer, fixture, branch, repoPath)
			...
			gitRepo.Namespace = "test-ns"
			gitRepo.Spec = sourcev1.GitRepositorySpec{
				URL:       repoURL,
				Reference: tt.gitRepoRef,
			}

			updateAuto := &imagev1.ImageUpdateAutomation{}
			updateAuto.Name = "test-update"
			updateAuto.Namespace = "test-ns"
			...
			kClient := fakeclient.NewClientBuilder().WithScheme(scheme.Scheme).WithObjects(gitRepo, updateAuto).Build()

			sm, err := NewSourceManager(context.TODO(), kClient, updateAuto, WithSourceOptionGitAllBranchReferences())
			...
			_, err = sm.CheckoutSource(context.TODO())
...
func test_sourceManager_CommitAndPush(t *testing.T, proto string) {
	tests := []struct {
		...
		// TODO: Add another test case that uses the ResultV2 updates.
		{
...
`,
	...
	for _, tt := range tests {
		t.Run(fmt.Sprintf("%s(%s)", tt.name, proto), func(t *testing.T) {
			...
			g.Expect(testutil.ReplaceMarker(workDir, policyKey))
