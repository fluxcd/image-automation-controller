...
func testdataPath(path string) string {
	return filepath.Join("..", "controller", "testdata", path)
}
...
func Test_applyPolicies(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			targetPolicyName: "policy1",
			replaceMarkerFunc: func(g *WithT, path string, policyKey types.NamespacedName) {
				g.Expect(testutil.ReplaceMarker(filepath.Join(path, "yes"), policyKey)).ToNot(HaveOccurred())
				g.Expect(testutil.ReplaceMarker(filepath.Join(path, "no"), policyKey)).ToNot(HaveOccurred())
			},
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.inputPath != "" {
				...
				if tt.replaceMarkerFunc != nil {
					...
					g.Expect(testutil.ReplaceMarker(workDir, targetPolicyKey)).ToNot(HaveOccurred())
				}
			}
			...
			if !tt.wantErr {
				...
				if tt.replaceMarkerFunc != nil {
					...
					g.Expect(testutil.ReplaceMarker(expected, targetPolicyKey)).ToNot(HaveOccurred())
				}
